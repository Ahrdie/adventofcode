...
for guard in guardTimes:
    ...
    for m in guardTimes[guard]:
        minuteSum = minuteSum + m
    if minuteSum > maxSleepSum:
        guardId = guard
        maxSleepSum = minuteSum

maxMinute = 0;
timesAsleep = 0;

for minute in guardTimes[guardId]:
    if minute > timesAsleep:
        maxMinute = guardTimes[guardId].index(minute)
        timesAsleep = minute

print "Elf " + str(guardId) + " sleeps most. Most often at 0:" + str(maxMinute) + ". Code: " + str(int(guardId[1:]) * maxMinute)
