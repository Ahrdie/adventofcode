...
generationToReach = 50000000000
generationsHigh = 2000000
potState = []
boolRange = [None,None]
paddingLeft = 0
...
smallGenerationSteps = 1000
bigGenerationSteps = 10000
lastPotCount = 0
lastPotCountDifference = 0
for generation in range(generationsHigh):

    currentPotCount = calculatePotCount()
    potDifference = currentPotCount - lastPotCount

    if lastPotCountDifference == potDifference:
        generationsLeft = generationToReach - generation
        generationStepsLeft = (generationToReach - generation) / bigGenerationSteps
        increasePerGeneration = potDifference
        extrapolatedGenerations = currentPotCount + generationsLeft * increasePerGeneration
        print "Extrapolated pot Value at generation " + str(generationToReach) + ": " + str(extrapolatedGenerations)
        break

    lastPotCount = currentPotCount
    lastPotCountDifference = potDifference
